...
func PriorityText(v string) string {
	m := map[string]string{constants.RepairPriorityNormal: "普通", constants.RepairPriorityUrgent: "紧急", constants.RepairPriorityMajor: "重大"}
	return m[v]
}

// ResponseDurationText 将首次响应耗时（秒）格式化为中文耗时，完成后保留并展示。
